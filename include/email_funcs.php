...
function CREATE_EMAIL_BODY(&$message,$replyto,$currentqueue,$site_name,$siteID,$page_name,&$con)
{
	...
	$phone_page = mysql_fetch_array(mysql_query("SELECT siteID FROM Sites Where SiteName = 'phoneshift'",$con));
	if ($_COOKIE['userID'] != '' AND !$userName) {
		$useractive = mysql_fetch_array(mysql_query("SELECT UserName FROM Users WHERE userID=".$_COOKIE['userID'].";",$con));
		$userName = "User-".$useractive['UserName'].", ";
	}
	...
	$message .= "Submitted by: ".$userName."Host-".gethostbyaddr($_SERVER['REMOTE_ADDR']).", IP-".$_SERVER['REMOTE_ADDR']."<br />\n";
